...
namespace Weddings.Controllers
{
    public class HomeController : Controller
    {
        ...
        //check databse if email already exists
            if(_context.user.Where(u => u.email == model.NewUser.email)
                                                  .ToList()
                                                  .Count() > 0) 
        ...
        public IActionResult Login(IndexView model)
        {
            ...
            if(ModelState.IsValid)
             {
                ...
                if(_context.user.Where(u => u.email == model.LogUser.log_email)
                                                  .ToList()
                                                  .Count() == 0)   
            ...
            //check hashed password if email exists in database
                ...
                else
                {
                    ...
                    User checkPassword = _context.user.SingleOrDefault(u => u.email == model.LogUser.log_email);
                    ...
                    //log in user
                    User user1 = _context.user.SingleOrDefault(person => person.email == model.LogUser.log_email);
